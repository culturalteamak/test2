...
//        dd($res);
            ...
            if ($peroid == '1min') {
                ...
                $data = MyQuotation::where('base',$base_currency)->whereBetween('itime',[$from,$to])->orderBy('itime','desc')->limit(500)->get();
                $data=$data->toArray();

                $res = [];
                foreach ($data as $_v) {
                    $_v['itime'] = strtotime($_v['itime']);
                    $res[] = [
                        'amount' => $_v['vol'],
                        'base_currency' => $_v['base'],
                        'close' => $_v['close'],
                        'high' => $_v['high'],
                        'low' => $_v['low'],
                        'open' => $_v['open'],
...
//                }
                ...
                $data = MyQuotationSum::where('base',$base_currency)->where('ktype',$peroid)->whereBetween('itime',[$from,$to])->orderBy('itime','desc')->limit(500)->get();
                $data=$data->toArray();
