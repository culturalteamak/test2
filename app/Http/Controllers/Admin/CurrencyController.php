...
class CurrencyController extends Controller
{
    ...
    public function postAdd(Request $request)
    {
        ...
        try {
            ...
            $currency->get_address=$get_address;
            // if ($key != '********' ) {
...
//        $account_number = $request->get('account_number','');
    ...
    public function postAddMatch(Request $request, $legal_id)
    {
        ...
        $change_trade_fee = $request->input('change_trade_fee', 0);
        $change_min_share = $request->input('change_min_share', 0);
        $change_max_share = $request->input('change_max_share', 0);
        ...
        $currency_match = CurrencyMatch::create([
            ...
            'change_trade_fee' => $change_trade_fee,
            ...
            'change_min_share' => $change_min_share,
            'change_max_share' => $change_max_share,
            'create_time' => time(),
        ]);
        CurrencyMatch::reguard();
        $name = Currency::getNameById($currency_id);
        $exists = HuobiSymbol::where('base-currency', strtolower($name))->where('symbol', strtolower($name . 'usdt'))->first();
        if (!$exists) {
            $hb = [];
            $hb['base-currency'] = strtolower($name);
            $hb['quote-currency'] = 'usdt';
//            $hb->pricePrecision = 8;
            $hb['price-precision'] = 8;
//            $hb->amountPrecision = 2;
            $hb['amount-precision'] = 2;
//            $hb->symbolPartition = 'main';
            $hb['symbol-partition'] = 'main';
            $hb['symbol'] = strtolower($name . 'usdt');
//            $hb->symbol =
           HuobiSymbol::insert($hb);
        }
