...
//        die;
    ...
    public function sendMail(Request $request)
    {
        ...
        $mode = Setting::getValueByKey('smtpmode', 2);
...
//            var_dump($mail->Host,$mail->Port);
            ...
            $mail->setFrom($mail_from_name,'no-reply');//设置邮件来源  //发件人
...
////            $smtp->debug = false;//是否显示发送的调试信息
            $state = $mail->send();//intval($mode) === 1 ? $mail->send() : $smtp->sendmail($smtpemailto, $smtpusermail, $mailtitle, $mailcontent, $mailtype);
            if ($state) {
                session(['code' => $code]);
    ...
    public function sendMailSafe(Request $request)
    {
