...
class WalletController extends Controller
{
    ...
    public function ieo(Request $request)
    {
        $currency_id = $request->post('currency');
        $currency = Currency::find($currency_id);
        $user_id = Users::getUserId();
        $amount = $request->post('amount');
        $currency_match = CurrencyMatch::where('legal_id', 3)->where('currency_id', $currency_id)->first();
//        var_dump($currency_match);
        if ($currency && $currency_match && $currency_match->market_from == 3) {
            ...
            try {
                ...
                if($user_legal->change_balance<$amount)
                {
                    return $this->error('Sorry, your credit is running low');
                }

                if ($user_legal && $user_currency) {

                    change_wallet_balance($user_legal, 2, -$amount, 22, '认购扣除');
                    change_wallet_balance($user_currency, 2, $amount / $price, 25, '认购增加');
                    IeoRecord::insert([
                        'created_at' => time(),
                        'amount' => $amount,
                        'scount' => $amount / $price,
                ...
                DB::commit();
                return $this->success('Buy successful!');
            } catch (\Exception $e) {
                DB::rollBack();
                return $this->error('Buy failed!');
            }


        }
    }
    public function IeoRecord(Request $request)
    {
        $ieo = IeoRecord::orderBy('created_at','desc')->limit(100)->get();
        return $ieo;
    }
    //删除提币地址
...
//资产划转
    ...
    public function postWalletOut()
    {
      ...
      // $bankname=Input::get('bankname','');
       // $bankno=Input::get('bankno','');
        // $bankuser=Input('bankuser','');
        $legal=Input::get('legalCurrency');
        ...
        // $userreal = UserRealHigh::where('user_id', $user_id)->first();
        // if(empty($userreal)||$userreal->review_status!=2)
        // {
        //     return $this->error('请先完成高级认证');
        // }
        try {
            DB::beginTransaction();
           ...
           // $walletOut->bankname=$bankname;
           // $walletOut->bankno=$bankno;
            $walletOut->legal=$legal;
           // $walletOut->bankuser=$bankuser;
            $walletOut->save();
    ...
    public function getMicroAmount(Request $request)
    {
        $user_id = $request->get('uid');
        $user = Users::find($user_id);
        if ($user) {
            $wallet = UsersWallet::where('user_id', $user_id)->where('currency', 3)->first();
            return $wallet ? $wallet->micro_balance : 0;
        } else {
            return 0;
        }
    }

    public function changeMicroAmount(Request $request)
    {
        $user_id = $request->get('uid');
        $user = Users::find($user_id);
        $amount = $request->get('amount');
        if (!is_numeric($amount)) {
            return $this->error('金额无效');
        }
        if ($user) {
            $wallet = UsersWallet::where('user_id', $user_id)->where('currency', 3)->first();
            if ($wallet) {
                $res = change_wallet_balance($wallet, 4, $amount, AccountLog::LTC_IN, '二元期权账户变更');
                if ($res) {
    ...
    public function getWalletAddressIn()
    {
        ...
        $address = $currencyInfo->contract_address;
        $count1 = WalletAddressLog::where(['user_id' => $user_id, 'currency_id' => $currency_id])->count();
        if ($count1 > 0) {
            $address = Db::table('users_wallet')->where(['currency' => $currency_id, 'user_id' => $user_id])->value('address');
        }
        return $this->success(['address' => $address,'qrcode'=>$currencyInfo->get_address]);
        die;
        $wallet_dic = [
            '1' => 'btc',
            '3' => 'usdt',
...
//    }
    ...
    public function legalLog(Request $request)
    {
        ...
        $lang_arr = ['kr' => 'kor', 'hk' => 'cht', 'jp' => 'jp', 'zh' => 'zh', 'en' => 'en', 'spa' => 'spa', 'de' => 'de'];
