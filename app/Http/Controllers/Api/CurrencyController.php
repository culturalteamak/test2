...
//        var_dump($rsp);
     ...
     */
    public function mobileQuotation(Request $request)
    {
        ...
        $currency = Currency::with('quotation')
            ...
            ->where('id',3)->where('is_display', 1)
            ->where('is_legal', 1)->first();

        if(!empty($sort) && !empty($updown)){

            if($updown == "down"){
                $currency->quotation =  $currency->quotation->sortByDesc($sort)->values()->toArray();

            }else{
                $currency->quotation =  $currency->quotation->sortBy($sort)->values()->toArray();
            }
