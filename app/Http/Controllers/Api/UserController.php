...
//        $user_id = Users::getUserId();
    ...
    public function infoMobile(Request $request){
        $user_id = Users::getUserId();
        ...
        $currency_name = $currency_usdt_id->name;
        ...
        $data = [
            'id'=>$user['id'],
            'head_url'=>'',
            'legal_id'=>$currency_usdt_id->id,
            'legal_name'=> $currency_usdt_id->name,
            ...
            "tradeCount"=>$legal_wallet['usdt_totle']+$change_wallet['usdt_totle']+$micro_wallet['usdt_totle']+$lever_wallet['usdt_totle'],
    ...
    public function info(Request $request)
    {
        ...
        $res1 = UserRealHigh::where('user_id', $user_id)->first();
        if(empty($res1))
        {
            $user['hreview_status'] = 0;
        }else{
            $user['hreview_status'] = $res->review_status;
        }
...
//            $userreal->hand_pic = $hand_pic;
    ...
    //高级认证
    public function realNameHigh()
    {

        $user_id = Users::getUserId();
        $passport = Input::get("passport", "");//真实姓名
        $jiashi = Input::get("jiashi", "");//身份证号
        $front_pic = Input::get("front_pic", "");//正面照片
        $passport_pic = Input::get("passport_pic", "");//反面照片
        $passport_hand_pic = Input::get("passport_hand_pic", "");//反面照片
        $jiashi_pic = Input::get("jiashi_pic", "");//正面照片
        $jiashi_hand_pic = Input::get("jiashi_hand_pic", "");//反面照片
//        $hand_pic = Input::get("hand_pic", "");//手持身份证照片
...
//        $userreal_number = UserRealHigh::where("user_id", $user_id)->count();
////        var_dump($userreal_number);die;
//        if ($userreal_number > 0) {
//            return $this->error("该身份证号已实名认证过!");
//        }
        ...
        try {
            ...
            $userreal = new UserRealHigh();

            $userreal->user_id = $user_id;
            $userreal->passport_id = $passport;
            $userreal->jiashi_id = $jiashi;
            $userreal->create_time = time();
            $userreal->front_pic = $front_pic;
            $userreal->passport_pic = $passport_pic;
            $userreal->passport_hand_pic = $passport_hand_pic;
            $userreal->jiashi_pic = $jiashi_pic;
            $userreal->jiashi_hand_pic = $jiashi_hand_pic;
//            $userreal->hand_pic = $hand_pic;
    ...
    public function userCenter()
    {
        ...
        $userrealh = UserRealHigh::where('user_id', $user_id)->first();

        if (empty($userrealh)) {
            $user['hreview_status'] = 0;
        } else {
            $user['hreview_status'] = $userrealh['review_status'];
