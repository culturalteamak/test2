...
class CapitalController extends Controller
{
    ...
    public function rechargeCount(Request $request)
    {
...
//        var_dump($items->toArray());
    ...
    public function wallettotalList(Request $request)
    {
        ...
        foreach ($list->items() as &$value) {
            ...
            $value->lever = UsersWallet::where('user_id',$user_id)->where('currency',$value->id)->sum('lever_balance');
            $value->change = UsersWallet::where('user_id',$user_id)->where('currency',$value->id)->sum('change_balance');
            $value->micro = UsersWallet::where('user_id',$user_id)->where('currency',$value->id)->sum('micro_balance');

            $value->_caution_money = LeverTransaction::where('user_id', $user_id)->whereIn('status', [0, 1, 2])->where('legal', $value->id)->sum('caution_money');
            $value->currency = LeverTransaction::where('user_id', $user_id)->whereIn('status', [0, 1, 2])->where('currency', $value->id)->sum('share');
            // foreach($value->currency->items() as &$value1){
            //     $value1->currency_name =  Currency::getNameById($value1->currency);
            // }
            // $value->currency = array_column($value->currency->toArray(),'currency_name');
        }
