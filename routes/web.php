...
Route::get('api/currency/quotation_mobile', 'Api\CurrencyController@mobileQuotation'); //币种列表带行情(支持交易对) 手机端
...
//    });
    ...
    Route::get('/api/front',function(Request $request){
        ...
        return [
            ...
            'default_invite' => 'hkrg',
            'validate_code' => true,
            'default_validate' => 9188,
            'site_name' => 'TDMock',
            'site_logo' => 'https://www.s-coin.vip/logo/logo.png',
            'show_download'=>true,
            'show_customer'=>true,
        ];
...
//<--LDH-->
    ...
    Route::post('/api/sms_mail_aq', 'Api\SmsController@sendMailSafe'); //获取邮箱验证码
...
//Route::any('api/currency/new_timeshar', 'Api\CurrencyController@newTimeshars')->middleware(['cross']); //K线分时数据，对接tradeingview
    ...
    Route::get("api/news/banner","Api\NewsController@getBanner");
    Route::get("api/news/getGG","Api\NewsController@getGG");
    Route::get("api/news/getTextsList","Api\NewsController@getTextsList");

    Route::prefix('api')->post('user/real_name', 'Api\UserController@realName')->middleware([
        'demo_limit',
        'check_api'
    ]);//身份认证
    Route::prefix('api')->post('user/real_name_high', 'Api\UserController@realNameHigh')->middleware([
        'demo_limit',
        'check_api'
    ]);//身份认证
    Route::get('api/wealth/getProfitList', 'Api\WealthController@getProfitList');
    Route::any('api/wealth/calc', 'Api\WealthController@calcProfit');
    Route::get('api/ieo/record','Api\WalletController@IeoRecord');
//<--LDHend-->
//******************************api接口需要登录的**********************
    Route::group(['prefix' => 'api', 'middleware' => ['check_api', /*'check_user'*/]], function () {
        ...
        Route::get('user/active', 'Api\UserController@activeTime');
        //反馈建议
        ...
        Route::post('wallet/ieo','Api\WalletController@ieo');
...
//     Route::get('/bank','Api\UserController@bankList');//获取收款方式
        ...
        Route::get('user/info/mobile', 'Api\UserController@infoMobile');//我的手机端
        ...
        Route::post('user/real_nameh', 'Api\UserController@realNameHigh');
        ...
        //持币生息
        Route::get('wealth/getProfitTotal', 'Api\WealthController@getProfitTotal'); //获取资产信息
         //获取产品列表
        Route::post('wealth/buyProfit', 'Api\WealthController@buyProfit'); //购买理财产品
        Route::post('wealth/sellProfit', 'Api\WealthController@sellProfit'); //赎回理财产品
        Route::get('wealth/getMyprofit', 'Api\WealthController@getMyprofit'); //我的理财产品
    ...
    Route::get('/admin/chargeCount',function(){
        $c = \App\ChargeReq::where('status',1)->count('id');
        $c1 = \App\ChargeReq::where('status',1)->whereBetween('created_at',[date('Y-m-d H:i:s',strtotime('-1 hour')),date('Y-m-d H:i:s')])->count('id');
        $w = \App\UsersWalletOut::where('status',1)->count('id');
        $w1 = \App\UsersWalletOut::where('status',1)->whereBetween('create_time',[strtotime('-1 hour'),time()])->count('id');
        $r = \App\UserReal::where('review_status',1)->count('id');
        $u = \App\Users::whereBetween('time',[strtotime(date('Y-m-d 00:00:00')),time()])->count('id');
        echo json_encode(['ok'=>1,'message'=>['c'=>$c,'c1'=>$c1,'w'=>$w,'w1'=>$w1,'r'=>$r,'u'=>$u]]);
    });
    Route::get('new/walletList', 'Api\WalletOneController@walletList');//

    Route::post('/admin/login', 'Admin\DefaultController@login');
    Route::get('/admin/myquotation/hangqing', 'Admin\MyQuotation@generate');
...
//管理后台
    Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
        ...
        Route::get('/winadmin/desktop','Admin\AdminController@menu');
        Route::get('/winadmin/start','Admin\AdminController@start');
        //持币生息管理
        Route::get('/wealth/product', 'Admin\WealthController@index');//持币生息管理
        Route::get('/wealth/product/list', 'Admin\WealthController@wealthLists');//持币生息产品列表
        Route::get('/wealth/product/add', 'Admin\WealthController@add');//持币生息增加产品
        Route::post('/wealth/product/add', 'Admin\WealthController@doadd');//持币生息增加产品
        Route::get('/wealth/product/edit', 'Admin\WealthController@edit');//持币生息修改产品
        Route::post('/wealth/product/edit', 'Admin\WealthController@doedit');//持币生息修改产品
        Route::post('/wealth/product/del', 'Admin\WealthController@del');//持币生息删除产品
        //持币生息交易
        Route::get('/wealth/trade', 'Admin\WealthTransactionController@index');//持币生息交易列表
        Route::get('/wealth/trade/lists', 'Admin\WealthTransactionController@lists');//持币生息交易列表
        ...
        Route::any('exchange/complete_index', 'Admin\Exchange@completeIndex');
        Route::post('exchange/complete_del', 'Admin\Exchange@completeDelete');
        Route::any('exchange/list', 'Admin\Exchange@list');
        Route::any('exchange/complete_list', 'Admin\Exchange@completeList');
        Route::post('exchange/revoke', 'Admin\Exchange@revoke');
        Route::post('exchange/deal', 'Admin\Exchange@deal');
        ...
        Route::post('myquotation/reset','Admin\MyQuotation@reset');
        Route::get('myquotation/preview','Admin\MyQuotation@preview');
...
//                rename($afile['tmp_'])
        ...
        //高级实名认证管理
        Route::get('/user/hreal_index', 'Admin\UserRealHighController@index');
        Route::get('/user/hreal_list', 'Admin\UserRealHighController@list');
        Route::get('/user/hreal_info', 'Admin\UserRealHighController@detail');
        Route::post('/user/hreal_del', 'Admin\UserRealHighController@del');
        Route::post('/user/hreal_auth', 'Admin\UserRealHighController@auth');
        ...
        Route::any('robote/add', 'Admin\RobotExchangeController@add');//添加机器人
        Route::any('robote/list', 'Admin\RobotExchangeController@list');//机器人列表
        Route::any('robote/list_data', 'Admin\RobotExchangeController@listData');//机器人列表
        Route::any('robote/delete', 'Admin\RobotExchangeController@delete');//删除机器人
        Route::any('robote/start', 'Admin\RobotExchangeController@start');//开启关闭机器人
        ...
        Route::post('/wallet/wash','Admin\WalletController@wash');
